...
function PartnerPortalLayoutInner({ children }: { children: React.ReactNode }) {
  ...
  const ownerNav = [
    ...
    { href: "/partner-portal/lizenzen", label: "Lizenzen", icon: (
      <svg className="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    { href: "/partner-portal/hetzner", label: "Hetzner", icon: (
      <svg className="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 12h14M5 12a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v4a2 2 0 01-2 2M5 12a2 2 0 00-2 2v4a2 2 0 002 2h14a2 2 0 002-2v-4a2 2 0 00-2-2" />
      </svg>
    )},
    { href: "/partner-portal/kunden/neu", label: "Neuer Kunde", icon: (
      <svg className="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  const sidebarContent = (isMobile = false) => (
    <>
      ...
      <nav className={`flex-1 py-5 space-y-0.5 ${collapsed && !isMobile ? "px-2" : "px-3"}`}>
        {navItems.map((item) => {
          const isActive = pathname === item.href || (item.href !== "/partner-portal/dashboard" && pathname.startsWith(item.href));
          return (
            <Link
              key={item.href}
              href={item.href}
